...
//     var  pResult   =  await  queryGraphApi( '/users/{id}/onenote/notebooks',  aToken,      'robin.mattern@sicomm.net' )
       ...
       var  aToken    =  pResult.token; if (pResult.value) {
            console.log( shoNotes2( pResult.value ) ) }  // token only ??
                         ...
                         await         shoAPI( '/groups/{id}/onenote/notebooks', aToken +'x',  shoNotes2, '5f7fabfc-9eea-4252-97f6-1b5b5de9a0c6','Bad Token' )
                         await         shoAPI( '/groups/{id}/onenote/notebooks', aToken,       shoNotes1, 'e7c1f766-1963-4a4b-8296-aebceeb24fb1','SicommNet Public Team' )
                         await         shoAPI( '/groups/{id}/onenote/notebooks', aToken,       shoNotes2, '5f7fabfc-9eea-4252-97f6-1b5b5___a0c6','Bad Group' )
...
//     var  pResult   =  await         shoAPI( '/joinedTeams',            '', fmtTeams3, '',                         'SicommNet, Inc' ); // Resource not found for the segment 'joinedTeams'.
       ...
       var  aToken    =  pResult.token
            ...
            pResult.data.map( async pTeam => {  // crazy!!  S.B. async function( pTeam ) { ...  }
                         await         shoAPI( '/groups/{id}/onenote/notebooks', aToken, shoNotes2, pTeam.TeamId, pTeam.Name )
                         } )
...
// -------  ------------------------------------------------------------------
  ...
  function  fmtTeams1( mData ) { return fmtTeams_1( mData, 'asData' ) }
  function  fmtTeams2( mData ) { return fmtTeams_2( mData, 'byFld'  ) }
  function  fmtTeams3( mData ) { return fmtTeams_2( mData, 'byRow'  ) }

  function  fmtTeams_1( mTeams,  asData ) {
//     var  aData =    asData ? asData : 'byRow'
    ...
    return  fmtData( mRecs, 'Teams', asData ) 

         }; // eof shoTeams1
// -------  ------------------------------------------------------------------

  function  fmtTeams_2( mTeams, asData ) {  // byRow

       var  aData =    asData ? asData : 'byRow', bData = (aData == 'asData')  
       var  mRecs =    mTeams.map( ( pRec , i ) => {
       var  pFlds = { 'No'         : take( -3, i + 1 ) + "."
                    , 'TeamName'   :           pRec.displayName
                    , 'Id'         :           pRec.id    
                    , 'CreatedOn'  : fmtDate(  pRec.createdDateTime + '' )
        ...
        if (bData) { return  pFlds } else {
            mFlds =  Object( pFlds ).keys.map( aFld => pFlds[ aFld ] )
    return  mFlds.join( "\n  " )
            } } )
...
// -------  ------------------------------------------------------------------
  ...
  function  shoTeams2( mTeams, asData ) {  // byRow

  function  fmtData( mRecs, aName, asData ) {
       var  aData =    asData ? asData : 'byRow'

        if (aData == 'asData' ) { return mRecs }

        if (aData == 'byFld'  ) { 
            mRecs =  mRecs.map( pFlds => Object( pFlds ).keys.map( aFld => `${ take( 20, aFld ) }: ${ pFlds[ aFld ] }` ) )
       var  aLine = "\n".padEnd( 53, "-" ) + "\n  "
       var  aRecs =  aLine + mRecs.join( aLine ) + aLine.replace( /[\n ]+$/, "" )
//          aRecs = `\n${aName}:` + aRecs
//   return aRecs        
        ...
        if (aData == 'byRow'  ) { 
            mRecs =  mRecs.map( pFlds => Object( pFlds ).keys.map( aFld => pFlds[ aFld ] ) ) 
       var  aLine = "\n  "
       var  aRecs =  aLine + mRecs.join( aLine )
//          aRecs = `\n${aName}:` + aRecs
            }
